...
| 命令 | 描述 |
| --------------- | ---------------------------------------------------------- |
| `search` | 搜索所有列以获取包含给定字符串的单词（模糊匹配） |
| `prefix` | 返回包含给定前缀的单词（精准词素匹配） |
| `suffix` | 返回包含给定后缀的单词（精准词素匹配） |
| `root` | 返回包含给定词根的单词（精准词素匹配） |
| `deri_suffix` | 返回包含给定派生后缀的单词（精准词素匹配） |
| `inf_suffix` | 返回包含给定屈折后缀的单词（精准词素匹配） |
...
#### 1. 词素级匹配 vs. 纯拼写匹配
* **Datamuse / Webster / OneLook:** 这些平台纯粹基于**单词的拼写字面量**进行匹配。如果您搜索以 `ion` 结尾 of the word（如使用 `*ion`），只要拼写是这几个字母结尾的单词都会被返回。
...
* **morph-query:** 查询的是数据库中的**词素切分标注数据列**。
  * *高精准度:* 搜索前缀 `un` 只会匹配 `un` 被标注为前缀的单词（例如 `un @@abandon`），避免了拼写干扰。
